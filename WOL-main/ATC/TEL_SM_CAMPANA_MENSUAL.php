...
//Conexión
require "../db/dbs.php";
  $query = "SELECT * FROM BITACORA.BITACORA_TEL_MCE_MES_AT WHERE (extract(month from FECHA_INSERT) = MONTH( current_date()));";
  ...
  $mi_host = $_SERVER["HTTP_HOST"];
    ...
    $myfile = fopen("TEL_MENSUAL_ANALYTICS_CLARO.json", "w") or die("Error archivo");
    $txt = $json_pretty;
    fwrite($myfile, $txt);
    fclose($myfile);
    //Leemos el archivo creado
    $JsonParser = file_get_contents("TEL_MENSUAL_ANALYTICS_CLARO.json");
...
//inserta bandera en bitacora
        ...
        $insert_bitacora = "INSERT INTO `BITACORA`.`BITACORA_TEL_MCE_MES_AT` (`FLAG`, `NOMBRE_JSON`) VALUES ('1', 'TEL_MENSUAL_ANALYTICS_CLARO');";
    ...
    while($id_1 <= $valorNetoTotal){
      ...
      foreach ($jsomp2["data"][$id_1] as $value1) {
       ...
       // INSERT DE LOS DATOS
        ...
        $sqlLog = "INSERT INTO `BITACORA`.`LOG_TEL_MENSUAL_AT` (`nombre_api`, `respuesta`, `respuesta_error`) 
        VALUES ('TEL_MENSUAL_ANALYTICS_CLARO', '".$RepInsertSF."', '".$RepInsertER."');";
