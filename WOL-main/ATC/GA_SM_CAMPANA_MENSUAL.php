...
//Conexión
require "../db/dbs.php";
  $query = "SELECT * FROM BITACORA.BITACORA_GA_CLARO_MES_AT WHERE (extract(month from FECHA_INSERT) = MONTH( current_date()));";
  ...
  $mi_host = $_SERVER["HTTP_HOST"];
    ...
    $myfile = fopen("GA_MENSUAL_ANALYTICS_CLARO.json", "w") or die("Error archivo");
    $txt = $json_pretty;
    fwrite($myfile, $txt);
    fclose($myfile);
    //Leemos el archivo creado
    $JsonParser = file_get_contents("GA_MENSUAL_ANALYTICS_CLARO.json");
...
//inserta bandera en bitacora
        ...
        $insert_bitacora = "INSERT INTO `BITACORA`.`BITACORA_GA_CLARO_MES_AT` (`FLAG`, `NOMBRE_JSON`) VALUES ('1', 'GA_MENSUAL_ANALYTICS_CLARO');";
      ...
      foreach ($jsomp2["data"][$id_1] as $value1) {
       ...
       // INSERT DE LOS DATOS
        ...
        //echo "Connected successfully";
         ...
         $sql = " INSERT INTO `DBCLARO`.`GA_CLARO_MES_AT` (`GA4 property`, `Event name`, `Session source`,
        ...
        VALUES ('GA_MENSUAL_ANALYTICS_CLARO', '".$RepInsertSF."', '".$RepInsertER."');";
